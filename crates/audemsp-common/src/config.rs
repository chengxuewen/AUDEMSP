//! Shared config schemas for all AUDEMSP components.
//!
//! Each component reads its YAML config file and deserializes into these types.

use serde::{Deserialize, Serialize};

/// Config for audemsp-host (capture + encode + push — field/vehicle side).
...
/// Config for audemsp-server (signaling + relay + monitoring).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Config for audemsp-client (pull + decode + control — cockpit/operator side).
#[derive(Debug, Clone, Serialize, Deserialize)]
