...
async fn main() {
    ...
    tracing::info!("AUDEMSP Remote v{} starting", env!("CARGO_PKG_VERSION"));
    ...
    let config_path = {
        ...
        if args.len() > 3 && args[2] == "--config" {
            ...
            "/opt/audemsp/etc/remote.conf".to_string()
        }
    };
...
let app = axum::Router::new()
    ...
    let psk = config.psk.as_deref().unwrap_or("audemsp-dev").to_string();
