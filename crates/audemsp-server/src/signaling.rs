...
use audemsp_common::auth::{JwtAuth, SimplePskAuth};
use audemsp_common::error::CoreError;
use audemsp_common::protocol::SignalingMessage;
use std::sync::Arc;
...
async fn handle_socket(socket: WebSocket, server: SignalingServer, jwt_token: Option<String>) {
    ...
    let psk = std::env::var("AUDEMSP_PSK").ok();
    ...
    match msg {
        SignalingMessage::CreateWebRtcTransport {
            ...
            let dir_str = match direction {
                ...
                audemsp_common::protocol::TransportDirection::Recv => "recv",
            };
