...
use audemsp_server::config;
use audemsp_server::monitor;
use audemsp_server::signaling;
use audemsp_server::admin;
use audemsp_common::auth::JwtAuth;
...
async fn run_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config_path = {
        ...
        if args.len() > 2 && args[1] == "--config" {
            ...
            "/opt/audemsp/etc/server.yaml".to_string()
    ...
    let app = axum::Router::new()
        ...
        .merge(admin_router);
    let app = audemsp_server::static_files::add_admin_routes(app);
    ...
    if let Some(ref tls_cfg) = config.tls {
        // ── TLS mode ──
        let rustls_config = audemsp_server::tls::build_rustls_config(tls_cfg)
            .await
