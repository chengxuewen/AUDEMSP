...
//! Run with: cargo test -p audemsp-webrtc --features backend-webrtc-sys --test webrtc_sys_tests
...
async fn factory_creates_with_webrtc_sys() {
    ...
    assert!(matches!(
        pc.connection_state(),
        audemsp_webrtc::peer_connection::RTCPeerConnectionState::New
    ));
    pc.close().await;
}
...
async fn close_state_transition() {
    ...
    assert!(matches!(
        pc.connection_state(),
        audemsp_webrtc::peer_connection::RTCPeerConnectionState::Closed
    ));
}
...
async fn ice_state_queries() {
    ...
    assert!(matches!(
        ...
        audemsp_webrtc::peer_connection::RTCIceConnectionState::New
    ));
    assert!(matches!(
        pc.ice_gathering_state(),
        audemsp_webrtc::peer_connection::RTCIceGatheringState::New
    ));
...
async fn signaling_state_default() {
    ...
    assert!(matches!(
        pc.signaling_state(),
        audemsp_webrtc::peer_connection::RTCSignalingState::Stable
    ));
