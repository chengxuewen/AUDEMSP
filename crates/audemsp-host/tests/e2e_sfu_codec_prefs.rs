...
//!
//! 前置 (C13/C21/C22):
//!   - server Docker 运行: docker compose up -d（含 AUDEMSP_SFU_ANNOUNCED_IP 宿主 IP）
...
use audemsp_webrtc::rtp::RTCRtpTransceiverDirection;
...
fn psk() -> String {
    std::env::var("SFU_E2E_PSK").unwrap_or_else(|_| "audemsp-dev".to_string())
}
...
fn build_multi_codec_sdp(
    ...
    ice_candidates: Option<&Vec<audemsp_common::protocol::IceCandidate>>,
) -> String {
