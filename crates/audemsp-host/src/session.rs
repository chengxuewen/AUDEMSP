...
/// Session state serialized to `/tmp/audemsp-host-session.json`.
...
impl Session {
    ...
    pub fn load(state_path: Option<&str>) -> Self {
        let path = PathBuf::from(
            state_path.unwrap_or("/tmp/audemsp-host-session.json"),
        );
    ...
    pub fn start_persist(&self) -> tokio::task::JoinHandle<()> {
        let path = PathBuf::from("/tmp/audemsp-host-session.json");
...
mod tests {
    ...
    fn session_load_missing_file_returns_fresh() {
        let session = Session::load(Some("/tmp/nonexistent-audemsp-session.json"));
    ...
    async fn session_start_persist_spawns() {
        ...
        let path = std::path::PathBuf::from("/tmp/audemsp-host-session.json");
