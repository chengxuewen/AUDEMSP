/* 车端信令示例（C ABI 消费）— ms_link_signal_* 用法。
 *
...
static void on_event(ms_link_signal_t* s, const char* event_json, void* user) {
    (void)s;
    (void)user;
    printf("event: %s\n", event_json);
}
...
int main(void) {
    ...
    ms_err_t rc = ms_link_signal_connect(&cfg, &s);
    if (rc != MS_OK) {
        ms_link_last_error(err, sizeof(err));
    ...
    ms_link_signal_on_event(s, on_event, NULL);
    ...
    rc = ms_link_signal_send(s, status, strlen(status));
    if (rc != MS_OK) {
        ms_link_last_error(err, sizeof(err));
