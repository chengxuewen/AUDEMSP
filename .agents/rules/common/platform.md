...
> This file: OS/platform-specific constraints for MediaServo development.

## Platform Constraints

### macOS Development — Host/Client Native, Server Docker
- **Host (`mediaservo-host`) and Client (`mediaservo-client`)**: Develop and run natively on macOS. These crates do not depend on mediasoup.
- **Server (`mediaservo-server`)**: Compile and run via Docker when `sfu-mediasoup` feature is enabled. The server binary and `cargo check` work natively on macOS, but mediasoup integration requires a Linux container.
...
- `.github/workflows/ci.yml` defines `test-mediasoup` with `runs-on: ubuntu-latest`. It installs meson, ninja-build, libuv1-dev, and libssl-dev before running `cargo test -p mediaservo-server --features sfu-mediasoup`.
