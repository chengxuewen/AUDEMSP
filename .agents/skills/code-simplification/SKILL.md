...
description: "Reduce complexity in OMSPBase Rust/TS code. Chesterton's Fence analysis, Rule of 500 enforcement, dead code elimination, borrow checker simplification patterns. Complements ponytail for Rust-specific over-engineering. Use after ponytail-audit, before PRs, or when diagnosing complexity smells."
---
...
文件: crates/omspbase-webrtc/src/backend/mod.rs
检测前: 487 行 → 检测后: 312 行 (-36%)

### 移除
- [行 45-78] 未使用 trait `LegacySdpParser` → 删除 (git blame: D78 引入, D112 废弃)
...
- [行 180-195] `#[cfg]` 三 backend 重复 — D15 架构代价，不能消除
- [行 300] `unsafe` buffer pool — 性能约束，见注释

### 验证
✅ cargo clippy -- -D warnings
✅ cargo test -p omspbase-webrtc
```
