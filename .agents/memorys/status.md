...
**生成**: 2026-08-06 | 决策: 187+ (D1-D212, 含跳号) | Phase: 3 完成 | 217 commits | 22 skills | mediasoup 0.24.1 | PIT-67

**当前**: 7 crate workspace (audemsp-*)。Phase 3 全部完成。**Host SFU 全链路实现完成** (host-webrtc-sfu-web-client 50/50: audemsp-webrtc 抽象层补全 + ICE/DTLS + E2E)。**统一 Docker 构建策略完成** (docker-build-strategy 29/29: 层缓存 + compose 分离 + Caddy 代理 + CI 镜像)。**OpenVidu 参考文档 3 篇**。Docker 环境本机已装 (29.1.3 + 镜像加速 + daemon 代理)。**构建优化 D208 已实施验证** — 本周 9 项 P0 修复全部完成：builder/dev/runtime 三 target 冒烟 EXIT 0 + runtime health 200；docker-cargo.sh 全链路实测通过（C13 check-server 首次可用，3m27s）；CI 门禁升级（PIT-39 gate 冒烟 + PR build-only）；沉淀 PIT-36~41（Docker dev 链路历史故障 + 编排/编辑教训）。**项目重命名 D209 完成**（OMSPBase→AUDEMSP，217 文件/2363 处）。**Host SFU Produce 打通 (PIT-54)**：根因 Host 手工 rtp_parameters 缺 H264 parameters → match_codecs 严格匹配失败；修复后 Producer 创建 + NewProducer 广播 + I420 帧循环运行。**浏览器 consume 渲染打通 (PIT-56/57)**：E2E videoWidth=640x480 + 棋盘格截图——全链路 Host 编码→mediasoup→浏览器完成；修复链=rtp_capabilities kind 字段+candidates 传递+setup:passive+本地指纹+offer sendonly（PIT-56 六连）+VideoFrame 时间戳递增（PIT-57 编码极小帧）。**人工测试环境就绪 (PIT-58)**：其他主机拉流 Signal Lost 修复——announced_address 配置化（AUDEMSP_SFU_ANNOUNCED_IP=宿主IP，192.168.2.127）；E2E 候选验证 192.168.2.127:20000 + 本机回归正常。**多色矩形帧交付 (PIT-62)**：Host 视频帧从调试棋盘格改为动态多色矩形（4 色块循环+彩色 U/V，E2E 640x480 通过）；VideoFrameGenerator/SquaresPattern 与 libwebrtc 兼容问题记录待办。**真实时间戳修复 (PIT-63)**：锚定单调 wall-clock 时间戳替换假时钟——E2E 640x480 + 关键帧间隔 11s→2.35s；T2.5 证实假时钟是停摆根因（mpsc 架构无问题）；PIT-60/61 记忆补齐。**SquaresPattern MVP 打通 (PIT-64/65)**：视频帧生成器图案经绝对时间轴修复（draw 耗时→帧率匹配）后人工验证渲染成功——生成器帧跑通全链路；E2E 连跑偶发不稳定待查（PIT-65）。**文档体系重组 D212 完成** — `docs/reference/` 按 Diátaxis 活参考/调研存档分离（C19）：活参考镜像模块 webrtc/ codec/ 平铺 + janus-gateway，28 篇竞品调研下沉 `research/<领域>/`；codec 验收标准迁入 `.sisyphus/plans/audemsp-codec/`；计划体系收敛单源 `.sisyphus/plans/`（移除已完成 video-framepipeline-hardening 双副本 + .omo/plans 去重，保留 phase3 约定 + host-sfu-w3c-alignment 待办）。**部署策略**: 优先本地构建（cargo-cache 卷 + 层缓存，初次 15-30min 一次性）；ghcr 预烘焙按需启动（团队 >1 人/换机频繁时再做，占位符 ghcr.io/org/audemsp-server 已就绪）。**待办**: admin dist 修复；host webrtc-p2p feature E0753 批量损坏（`//!` 在 cfg 后，13 处，既有）；PIT-65 E2E 连跑不稳定（Squares 流，传输层待查）；host webrtc-p2p feature E0753 批量损坏（`//!` 在 cfg 后，13 处，既有）；PIT-65 E2E 连跑不稳定（Squares 流，传输层待查）；audemsp-webrtc 补 `get_rtp_parameters(track_id)` API（PIT-54，Host 手工 rtp_parameters 双硬编码消除）
