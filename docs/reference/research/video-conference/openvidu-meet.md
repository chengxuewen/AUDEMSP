...
客户端不直接操作 RTCPeerConnection，所有 WebRTC 细节由 SDK 封装。MediaServo Admin Dashboard 可以定义 `MeetingSession` 接口，封装 join/leave/publish/subscribe，底层实现可切换（P2P、SFU-mediasoup、SFU-LiveKit），对 UI 层完全透明。

### 9.2 Token 鉴权流程

应用服务器生成 Token，客户端不接触 REST API 密钥。每个 Token 绑定特定 Session 和 Connection，支持角色权限控制（PUBLISHER/SUBSCRIBER），单次使用防重放。MediaServo 的 SFU 鉴权可以借鉴此设计，替代当前简单的 PSK 模型。

### 9.3 Subscriber 自动管理 + 信号消息通道

streamCreated 事件自动触发 subscribe，streamDestroyed 自动触发 unsubscribe，事件驱动而非轮询。MediaServo 的 SFU consume 流程可以封装为类似的响应式模式。

signal() 机制提供轻量级自定义消息通道，不需要额外连接。MediaServo 的 WebSocket 信令协议可以增加类似的通用信号通道。
...
| 维度 | OpenVidu Meet | MediaServo Admin Dashboard |
|------|---------------|-------------------------|
...
OpenVidu Meet 是成熟的参考实现，MediaServo Admin Dashboard 可以从中提取大量设计模式，避免重造轮子。