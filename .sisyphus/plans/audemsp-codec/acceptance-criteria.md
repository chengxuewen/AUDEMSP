# SDD: mediaservo-codec Acceptance Criteria
...
2. Verify with:
   nm target/release/mediaservo-client | grep " U " | grep -E "avcodec_|avformat_|avutil_|sws_"
   # Should be empty — all FFmpeg symbols resolved (no Undefined)
3. Verify no dlopen:
   strings target/release/mediaservo-client | grep -E "dlopen|dlclose|dlsym"
   # Should be empty
...
- **Audio codec** support (OPUS, AAC) — belongs to `mediaservo-webrtc::rtp` layer.
...
| `docs/superpowers/specs/2026-07-20-mediaservo-media-crate-design.md` | Base types and traits |
